...
class Subjecttimetable_model extends MY_Model {
    ...
    public function getBySubjectGroupDayClassSection($subject_group_id, $day, $class_id, $section_id) {
        ...
        $this->db->select('subject_timetable.*,lessontimes.time_from,lessontimes.time_to,lessontimes.time_type');
        $this->db->from('subject_timetable');
        $this->db->join('subject_group_subjects', 'subject_timetable.subject_group_subject_id = subject_group_subjects.id');
        $this->db->join('staff', 'staff.id = subject_timetable.staff_id');
        $this->db->join('lessontimes', 'lessontimes.id = subject_timetable.lesson_id');
        ...
        $this->db->order_by('subject_timetable.lesson_id', 'asc');
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function getSubjectByClassandSectionDay($class_id, $section_id, $day) {
        ...
        $sql = "SELECT `subject_group_subjects`.`subject_id`,subjects.name as `subject_name`,subjects.code,subjects.type,staff.name,staff.surname,staff.employee_id,
        `subject_timetable`.* FROM `subject_timetable` 
        ...
        WHERE `subject_timetable`.`class_id` = " . $class_id . " AND 
        `subject_timetable`.`section_id` = " . $section_id . " AND 
        `subject_timetable`.`day` = " . $this->db->escape($day) . " AND 
        `subject_timetable`.`session_id` = " . $this->current_session . "" . $subject_condition;
    ...
    public function validateSubjectCountbyWeek($classId, $SectionId)
    {
        $sessionId = $this->current_session;

        $sql = "SELECT subjects.id, subjects.name, subjects.code, COUNT(subject_timetable.id) AS subjects_count, subjects.`sub_on_weekly`
                        FROM `subject_timetable` 
                    JOIN `subject_group_subjects` ON `subject_timetable`.`subject_group_subject_id` = `subject_group_subjects`.`id` 
                    INNER JOIN subjects ON subject_group_subjects.subject_id = subjects.id 
                    WHERE `subject_timetable`.`class_id` = '$classId' AND 
                    `subject_timetable`.`section_id` = '$SectionId' AND 
                    `subject_timetable`.`session_id` = '$sessionId' 
                    GROUP BY subjects.id
                    HAVING subjects_count<>subjects.`sub_on_weekly`";
        $return_validate = $this->db->query($sql);
        return $return_validate->result();
    }


    public function validateLessonCountbyWeek($classId, $SectionId)
    {
        $sessionId = $this->current_session;

        $sql = "SELECT COUNT(id) AS lesson_count
                        FROM `subject_timetable` 
                    WHERE `subject_timetable`.`class_id` = '$classId' AND 
                    `subject_timetable`.`section_id` = '$SectionId' AND 
                    `subject_timetable`.`session_id` = '$sessionId' ";
        $return_validate = $this->db->query($sql);
        //print($sql); die;
        return $return_validate->result()[0]->lesson_count;
    }    
    public function validateDuplicatedStaffInAtime($staffId=0)
    {
        $sessionId = $this->current_session;

        $sql = "SELECT  subject_timetable.`lesson_id`
                        ,staff.name,staff.surname,staff.employee_id
                        ,COUNT(staff.id) AS staff_count
                        ,GROUP_CONCAT(`classes`.`class`,':',sections.`section` SEPARATOR ',') AS dup_classes
                        ,`subject_timetable`.`day` AS dup_days
                        ,CONCAT(`lessontimes`.`time_from`,'~ ',`lessontimes`.`time_to`) AS dup_time
                            FROM `subject_timetable` 
                            INNER JOIN staff ON staff.id=subject_timetable.staff_id   
                            LEFT JOIN `lessontimes` ON `lessontimes`.id=subject_timetable.`lesson_id`
                        LEFT JOIN classes ON classes.id=`subject_timetable`.`class_id`
                        LEFT JOIN sections ON sections.id=`subject_timetable`.`section_id`
                            WHERE 
                            `subject_timetable`.`session_id` = '$sessionId' AND 
                            staff.is_active=1 AND subject_timetable.lesson_id<>0
                ";
         $sql .=  (!empty($staffId) ? " AND subject_timetable.staff_id = '$staffId'" : "");
         $sql .= "
                            GROUP BY `subject_timetable`.`day`,subject_timetable.`lesson_id`, staff.id
                            HAVING staff_count > 1
                            ORDER BY CASE WHEN LOWER(`subject_timetable`.`day`)='monday' THEN 1 ELSE
                                    CASE WHEN LOWER(`subject_timetable`.`day`)='tuesday' THEN 2 ELSE
                                    CASE WHEN LOWER(`subject_timetable`.`day`)='wensday' THEN 3 ELSE
                                    CASE WHEN LOWER(`subject_timetable`.`day`)='thursday' THEN 4 ELSE
                                    CASE WHEN LOWER(`subject_timetable`.`day`)='friday' THEN 5 ELSE
                                    CASE WHEN LOWER(`subject_timetable`.`day`)='saturday' THEN 6 ELSE
                                    CASE WHEN LOWER(`subject_timetable`.`day`)='sunday' THEN 7 ELSE 8
                                END  END END END END END END 
                            , `subject_timetable`.`day`,subject_timetable.`lesson_id` ";
       // print($sql); exit;
        $return_validate = $this->db->query($sql);
        return $return_validate->result();
    }

    public function getBySubjectGroupClassSection($subject_group_id, $class_id, $section_id) {
    ...
    public function getSubjectForPrintTeacherTimeTable($subject_timetable_id) {
        $sql = "";
        $sql .= " SELECT subjects.* ";
        $sql .= " FROM subject_timetable ";
        $sql .= " INNER JOIN subject_group_subjects ON subject_group_subjects.id = subject_timetable.subject_group_subject_id ";
        $sql .= " INNER JOIN subjects ON subjects.id = subject_group_subjects.subject_id ";
        $sql .= " WHERE subject_timetable.id = ".$subject_timetable_id;
