...
class Staff_model extends MY_Model
{
	...
	public function get_permission($id, $level_id=0)
	{
		$this->db->select("staff.id as staff_id, staff_roles.is_permission,staff_roles.id");
		$this->db->from("staff");
        ...
        if( !empty($id))		        $this->db->where("staff_roles.id", $id);
        if( !empty($level_id))		    $this->db->where("level_class.level_id", $level_id);
        $this->db->group_by("staff.id");

		$query = $this->db->get();
        //print($this->db->last_query()); die;
        if($id != 0)
		    return $query->row()->is_permission;
        else
            return $query->result_array();
	}
    ...
    {
        $this->db->where('id', $data['id'])->update('staff_roles', $data);
