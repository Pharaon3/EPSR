...
class Classsection_model extends MY_Model {
    ...
    public function updateLevel($data) {
        ...
        if (isset($data['class_id'])) {
            ...
            $query = $this->db->get('level_class');
            if ($query->num_rows() > 0) {
                $this->db->where('class_id', $data['class_id']);
                $this->db->update('level_class', $data);
            } else {
                $this->db->insert('level_class', $data);
            }
        }
    }

    public function add($data, $sections, $level = null) {
        ...
        if (isset($data['id'])) {
            ...
            if(isset($level)){
                $level['class_id'] = $class_id;
                $this->db->insert('level_class', $level);
            }
    ...
    public function getByID($id = null) {
        $this->db->select('classes.*, levels.level, level_class.level_id')->from('classes')->join('level_class', "level_class.class_id = classes.id","left")->join('levels',"levels.id = level_class.level_id", "left");
        ...
        if ($id != null) {
            ...
            if (!empty($vehicle_routes)) {
                foreach ($vehicle_routes as $vehicle_key => $vehicle_value) {
                    ...
                    $vec_route->level_id = $vehicle_value['level_id'];
            ...
            if (!empty($vehicle_routes)) {
                foreach ($vehicle_routes as $vehicle_key => $vehicle_value) {
                    ...
                    $vec_route->level = $vehicle_value['level'];
