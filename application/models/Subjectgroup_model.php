...
class Subjectgroup_model extends MY_Model {
        ...
        if (isset($role_id) && ($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
            if ($userdata["class_teacher"] == 'yes') {
                ...
                // } else {
                    ...
                    if(count($my_subjects)==0 || strlen($my_subjects['subject'])==0)  $my_subjects = "0";
                    $subject_groupid_condition = " and subject_group_subjects.id in(" . $my_subjects['subject'] . ")";
                // }
            }
        }

        $sql = "SELECT subject_group_subjects.*,subjects.name,subjects.code,subjects.type FROM `subject_group_subjects`
                INNER JOIN subjects on subjects.id=subject_group_subjects.subject_id
                WHERE subject_group_id =" . $this->db->escape($subject_group_id) . "
                and session_id =" . $this->db->escape($this->current_session) . "" .
                $subject_groupid_condition. " ORDER BY subjects.order desc";
        $query = $this->db->query($sql);
    ...
    public function getGroupByClassandSection($class_id, $section_id) {
        ...
        if ($return) {
            $sql = "SELECT subject_groups.name, subject_group_class_sections.* from subject_group_class_sections 
            ...
            and subject_groups.session_id=" . $this->db->escape($this->current_session) . " " . $subject_groupid_condition . " ORDER by subject_groups.id DESC";
            $query = $this->db->query($sql);
    ...
    public function getsubjectByclassidAndPeriod($class_id)
    {
        $sql = "SELECT  subjects.name FROM grading_subject_results 
                ...
                INNER JOIN subjects ON subjects.id = subject_group_subjects.subject_id 
                WHERE student_session.class_id = ' $class_id' and student_session.session_id = ".$this->db->escape($this->current_session)."
                 GROUP BY subjects.name order by subjects.name ASC";
        // $sql = "SELECT subjects.name FROM `subject_group_class_sections`
        //         INNER JOIN subject_groups on subject_groups.id=subject_group_class_sections.subject_group_id
        //         INNER JOIN subject_group_subjects on subject_group_subjects.subject_group_id=subject_groups.id
        //         INNER JOIN subjects on subjects.id=subject_group_subjects.subject_id
        //         WHERE subject_group_class_sections.class_section_id =" . $class_id . "
        //         and subject_group_class_sections.session_id =" . $this->db->escape($this->current_session) . "ORDER BY subject_group_class_sections.id asc";
        // print($sql);
        // exit();
        $query = $this->db->query($sql);
        return $query->result();
        ...
        // and subject_groups.session_id=" . $this->db->escape($this->current_session) . " group by subjects.id ORDER by subject_groups.id DESC ";
        // $query = $this->db->query($sql);
        // return $query->result_array();
    }
    ...
    function get_subjectgroup_subject_id($subject_group_id, $subject_id) {

        $this->db->where('session_id', $this->current_session);
        $this->db->where('subject_group_id', $subject_group_id);
        $this->db->where('subject_id', $subject_id);

        $query = $this->db->get('subject_group_subjects');
        if ($query->num_rows() > 0) {
