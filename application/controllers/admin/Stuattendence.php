...
class Stuattendence extends Admin_Controller {
    ...
    function attendencebyMonth() {
        if (!$this->rbac->hasPrivilege('student_attendance', 'can_view')) {
            access_denied();
        }

        $this->session->set_userdata('top_menu', 'Attendance');
        $this->session->set_userdata('sub_menu', 'stuattendence/attendencebyMonth');
        $data['title'] = 'Add Fees Type';
        $data['title_list'] = 'Fees Type List';
        $sch_setting = $this->setting_model->getSchoolDetail();
        $data['sch_setting'] = $this->sch_setting_detail;
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/stuattendence/attendenceListMonth', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $class = $this->input->post('class_id');
            $section = $this->input->post('section_id');
            ...
            $data['section_id'] = $section;
            $data['month'] = $month;
            $dates = $this->getDatesInMonth($month);
            $search = $this->input->post('search');
            $holiday = $this->input->post('holiday');
            if ($search == "saveattendence") {
                $session_ary = $this->input->post('student_session');
                ...
                foreach ($session_ary as $key => $value) {
                    foreach ($dates as $eachdate) {
                        list($dateString1, $dateString) = explode('/', $eachdate);
                        $checkForUpdate = $this->input->post('attendendence_id' . $value . '_' . $dateString);
                        $attendance_type = $this->input->post('attendencetype');
                        if ($attendance_type == '') $attendance_type = $this->input->post('attendencetype' . $value);
                        if ($attendance_type == '') $attendance_type = $this->input->post('studentattendanceselection_' . $value . '_' . $dateString);
                        if ($checkForUpdate != 0) {
                            if (isset($holiday)) {
                                $arr = array(
                                    'id' => $checkForUpdate,
                                    ...
                                    'attendence_type_id' => 5,
                                    'remark' => $this->input->post("remark" . $value),
                                    'date' => date('Y-m-d', $this->customlib->datetostrtotime($month))
                                );
                            } else {
                                $arr = array(
                                    'id' => $checkForUpdate,
                                    ...
                                    'remark' => $this->input->post("remark" . $value),
                                    'date' => date('Y-m-d', $this->customlib->datetostrtotime($dateString . '/' . $month))
                                );
                            }
                            $insert_id = $this->stuattendence_model->add($arr);
                        } else {
                ...
                if (!empty($absent_student_list)) {
                    ...
                    $this->mailsmsconf->mailsms('absent_attendence', $absent_student_list, $month);
                }

                $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>');
                redirect('admin/stuattendence/attendencebyMonth', 'refresh');
            }
            $attendencetypes = $this->attendencetype_model->get();
            $data['attendencetypeslist'] = $attendencetypes;
            $resultlist = $this->stuattendence_model->searchAttendenceClassSectionByMonth($class, $section, date('Y-m', $this->customlib->datetostrtotime('1/' . $month)));
            $stuattendanceList = [];
            foreach ($resultlist as $attendance) {
                $temp_stu_id = $attendance['std_id'];
                list($year, $month1, $day) = explode('-', $attendance['date']);
                $current_index = count($stuattendanceList);
                foreach ($stuattendanceList as $index => $stuattendance) {
                    if ($stuattendance['std_id'] == $temp_stu_id) {
                        $current_index = $index;
                    }
                }
                if ($current_index == count($stuattendanceList)) {
                    $stuattendanceList[] = $attendance;
                }
                $stuattendanceList[$current_index]['attendence_type_id_' . $day] = $attendance['attendence_type_id'];
                $stuattendanceList[$current_index]['att_type_' . $day] = $attendance['att_type'];
                $stuattendanceList[$current_index]['attendence_id_' . $day] = $attendance['attendence_id'];
            }
            $data['resultlist'] = $resultlist;
            $data['stuattendanceList'] = $stuattendanceList;
            $data['dates'] = $dates;
            $this->load->view('layout/header', $data);
            $this->load->view('admin/stuattendence/attendenceListMonth', $data);
            $this->load->view('layout/footer', $data);
    ...
    function getDatesInMonth($month_year) {
        ...
        for ($day = 1; $day <= $totalDays; $day++) {
            ...
            // Check if the day is a weekday (1 = Monday, 7 = Sunday)
            if ($date->format('N') < 6) {
                // Format the date as "M 01/03"
                $formattedDate = $date->format('D') . ' ' . $date->format('m/d');
                $dates[] = $formattedDate;
            }
        }
