...
class Subject extends Admin_Controller {
    ...
    function index() {
        ...
        $this->form_validation->set_rules('sub_on_weekly', $this->lang->line('type'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('code', $this->lang->line('subject_code'), 'trim|required|callback__check_code_exists');
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                ...
                'sub_on_weekly' => $this->input->post('sub_on_weekly'),
            );
    ...
    function edit($id) {
        ...
        $this->form_validation->set_rules('code', $this->lang->line('subject_code'), 'trim|required|callback__check_code_exists');
        $this->form_validation->set_rules('sub_on_weekly', $this->lang->line('sub_on_weekly'), 'trim|required|callback__check_code_exists');
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/header', $data);
    ...
    function update_sub_onWeekly()
    {
        $sub_on_weekly = $this->input->post('sub_on_weekly');
        $data['sub_on_weekly'] = $sub_on_weekly;
        $this->subject_model->update_sub_on_weekly($data);
        redirect('admin/subject/index');
    }
