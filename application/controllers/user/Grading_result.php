...
class Grading_result extends Student_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('user/gradingreport/main', $data);
...
//teachers deliverable grading reports to coordinator;
    ...
    public function viewnewreport($id, $period_id = null)
    {
        try{
            ...
            $level_id=$class_level['level_id'];
            if( empty($level_id) ) $level_id=43;
            $period_list = $this->Gradingreport_model->getPeriodEnabledByStaffId(4, $level_id);
            $data['periodList'] = $period_list;
            $data['isPrekender'] = $class_level['level_id'] != 43 ? true : false;
            ...
            $this->load->view('user/gradingreport/grading_newreport', $data);
        }
        catch (Exception $e){
            echo json_encode($e);
        }
            ...
            if (!empty($id)) {
                ...
                foreach ($studentList as $student) {
                    ...
                    if ($data['isPrekender'] && !$data['is_primary']) {
                        ...
                        $data['valuescaleList'] = $valuescale;
                        $data['order_number'] = $order_number;
                        $student_admit_cards = $this->load->view('user/gradingreport/newreportview', $data, true);
                        $renderprintpage .= " " . $student_admit_cards . " ";
                        $order_number++;
                    } else if ($data['is_primary']) {
                        $subjects = array();
                        ...
                        $data['pageNum'] = $this->session->userdata['pageNum'];
                        $data['grading_subject_results'] = $grading_subject_results;
                        $seconday_grading_report_cards = $this->load->view('user/gradingreport/_primary_report_view', $data, true);
                        $renderprintpage .= " " . $seconday_grading_report_cards . " ";
                    } else {
                        $subjects = array();
                        $subject_groups = $this->subjectgroup_model->getGroupByClassandSection($student['class_id'], $student['section_id']);
                        ...
                        $data['pageNum'] = $this->session->userdata['pageNum'];
                        $data['grading_subject_results'] = $grading_subject_results;
                        $seconday_grading_report_cards = $this->load->view('user/gradingreport/_secondarynewreportview', $data, true);
                        $renderprintpage .= " " . $seconday_grading_report_cards . " ";
                    }
                }
