...
class Student extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->sch_setting_detail->adm_auto_insert) {
            ...
            $prefix = explode('-',$session_array['session'])[0];
            if ($this->sch_setting_detail->adm_auto_insert) {
                if ($this->sch_setting_detail->adm_update_status) {
                    ...
                    $last_student         = $this->student_model->lastRecord($prefix);
        ...
        if ($this->form_validation->run() == false) {
            ...
            $admission_no                      = 0;
            if (0)//$this->sch_setting_detail->adm_auto_insert) 
            {
                if ($this->sch_setting_detail->adm_update_status) {
                    $admission_no = $this->sch_setting_detail->adm_prefix . $this->sch_setting_detail->adm_start_from;
                    $last_student         = $this->student_model->lastRecord($prefix);
            ...
            if ($insert) {
             ...
             //   );
                ...
                $student_login_detail = array('id' => $insert_id, 'credential_for' => 'student', 'username' => $admission_no, 'password' => $admission_no, 'contact_no' => $this->input->post('mobileno'), 'email' => $this->input->post('email'));
                ...
                if ($sibling_id > 0) {
                    ...
                    $parent_login_detail = array('id' => $insert_id, 'credential_for' => 'parent', 'username' => 'padre.' . $admission_no, 'password' => $admission_no, 'contact_no' => $this->input->post('guardian_phone'), 'email' => $this->input->post('guardian_email'));
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            if (!empty($insert_id)) {
                               ...
                               //     'password' => $student_data[$i]["admission_no"],
                               //     'user_id'  => $insert_id,
                               //     'role'     => 'student',
                               // );
                             ...
                             //   $data_parent_login = array(
                             //       'username' => $this->parent_login_prefix . $student_data[$i]["admission_no"],
                               //     'password' => $student_data[$i]["admission_no"],
                                 //   'user_id'  => $insert_id,
                                   // 'role'     => 'parent',
                                    //'childs'   => $temp,
                             //   );
                                ...
                                $student_login_detail = array('id' => $insert_id, 'credential_for' => 'student', 'username' => $student_data[$i]["admission_no"], 'password' => $student_data[$i]["admission_no"], 'contact_no' => $mobile_no, 'email' => $email);
                                $this->mailsmsconf->mailsms('login_credential', $student_login_detail);
                                ...
                                $parent_login_detail = array('id' => $insert_id, 'credential_for' => 'parent', 'username' => 'padre.'. $student_data[$i]["admission_no"], 'password' => $student_data[$i]["admission_no"], 'contact_no' => $guardian_phone, 'email' => $guardian_email);
