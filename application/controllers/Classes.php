...
class Classes extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->classsection_model->add($class_array, $sections, $level_array);
    ...
    public function delete($id)
    {
        ...
        $this->Level_model->deleteLevelClass($id);
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $level_id      = $this->input->post('pre_level_id');
            ...
            if($this->input->post('level_id') != $level_id ){
                $level_array = array(
                    'class_id'    => $class_id,
                    'level_id' => $this->input->post('level_id'),
                );
                $this->classsection_model->updateLevel($level_array);
            }
    ...
    {
        $class_section_id_array = $_REQUEST["class_section_id"]; 
        
        foreach($class_section_id_array as $cs_id=>$ampm_flag)
        {
            $this->class_model->UpdateTimezone($cs_id, $ampm_flag);
        }
        redirect('classes/timezone');
    }
    ...
    public function timezone()
    {
        ## menu activate
        $this->session->set_userdata('top_menu', 'Academics');
        $this->session->set_userdata('sub_menu', 'classes/timezone');
        //$data['lesson_timezone'] = $this->timetable_model->gettimezone();
        $data['timezone'] = $this->class_model->get_timezone_list();
        //print_r($data['timezone']); die;
        $this->load->view('layout/header', $data);
        $this->load->view('class/timezone_list', $data);
        $this->load->view('layout/footer', $data);
