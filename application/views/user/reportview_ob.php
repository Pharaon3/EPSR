...
<div class="mark-container" style="position:relative; border:solid 0px;height: 1500px;margin-top: 10px;">
    ...
    <?php
        ...
        if($level == "NIVEL INICIAL")
            $height = 1200;
        if($currentpagesize >= $height || ((mb_strlen($observation) > 190 && $currentpagesize > 1000) || (mb_strlen($observation) > 600 && $currentpagesize > 800)))
        {
            echo "</tr></table>";
            echo  "</div>";
            echo '<div style="width:100%;position:absolute;margin-top: 1470px;widht:100%;height: 20px;z-index: 1;border: solid 0px;border-top:solid 1px; ">
        ...
        echo '<div style="line-height:1.5; width:100%;text-align: left;">
            ...
            $i = 0;
            if(mb_strlen($observation) >= 100)
            {
                $str = mb_substr($observation,$i,100);
            ...
            {
                $path = getcwd().'/application/views/admin/gradingreport/arial.ttf';
                $font_size = 3; // Font size is in pixels.
                $aaa = 0;
                while(mb_strlen($str) != 0)
                {
                    $subsize = 95;
                    $image_width = 0;
                    while($image_width < 183)
                    {
                        $str = mb_substr($observation,$i,$subsize);
                        if($i + $subsize >= mb_strlen($observation)) break;
                        $type_space = imagettfbbox($font_size, 0, $path, $str);
                        $image_width = abs($type_space[4] - $type_space[0]);
                        $subsize++;
                    }
                  
                    $i += mb_strlen($str);
                    echo '<div style="width:100%;font-size:25px;padding-left:5px; margin-top:10px; border-bottom: 1px solid black;">'.$str.'</div><br>';
                    if($i >= mb_strlen($observation) - 1)
                        break;
                }
            }
        echo '<div style="width:100%;margin-top:20px;"></div>'; 
            echo '<div style="float:left;width:20%; font-size:30px; height: 30px; top:15px;font-weight: bold;">Promovido/a a:</div>'; 
            echo '<div style="float:right;width:80%; height: 30px; padding-top:10px; border-bottom: 1px solid black;"></div>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
        echo '<div style="width:100%;position:absolute;margin-top: 1470px;widht:100%;height: 20px;z-index: 1;border: solid 0px;border-top:solid 1px; ">
