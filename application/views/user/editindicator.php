<div class="content-wrapper">
    ...
    <section class="content">
        <div class="row">
            <?php
            if ($this->rbac->hasPrivilege('grading_report_indicators', 'can_eidt')) {
            ?>
                <div class="col-md-4">
                        ...
                        <form id="indicators_form" name="indicators_form" method="post" accept-charset="utf-8">
                            <div class="box-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="hidden" id="indicators_competence_id" name="indicators_competence_id" value="<?php echo $competence_id; ?>" >
				...
				<div class="form-group">
					...
					<?php
                                        ...
                                        ?>
                                            <option <?php if ($competence_id == $competence["id"])  echo "selected"; ?> value="<?php echo $competence['id'] ?>"><?php echo $competence['name'] ?></option>
                                        <?php
                                        }
                                        ?>
                                    </select>
                                ...
                                <br><br>
                                <div class="form-group">
                                    <?php if ($this->rbac->hasPrivilege('grading_report_indicators', 'can_add')) { ?>
                                ...
                                <?php foreach ($indicators as $indicator) { ?>
                                    <div class="form-group" id="<?php echo $indicator['id']; ?>">
                                        <label><?php echo $this->lang->line('indicators') . " " . $this->lang->line('name'); ?></label><small class="req"> *</small>
                                        <input type="text" name="indicators_<?php echo $indicator['id']; ?>" value="<?php echo $indicator['name']; ?>" style="width:95%" /> 
                                        <?php if ($this->rbac->hasPrivilege('grading_report_indicators', 'can_delete')) { ?>
                                            <span onclick="delete_indicators('<?php echo $indicator['id']; ?>')" class="text-danger">&nbsp;<i class="fa fa-remove"></i></span>
                                        <?php } ?>
                                    </div>
                                <?php } ?>

                                <div id="indicators_result"></div>
                                <div id="indicators_delete"></div>
                            </div>
...
<script>
    ...
    function add_indicators() {
        var id = makeid(8);
        $('#indicators_result').append('<div class="form-group" id="' + id + '"><label><?php echo $this->lang->line("indicators") . ' ' . $this->lang->line('name'); ?></label><small class="req"> *</small><br><input type="text" name="indicators[]" class="lessinput" /> <span  onclick="cancel_indicators(' + id + ')" class="section_id_error text-danger">&nbsp;<i class="fa fa-remove"></i></span></div>');
    }

    function cancel_indicators(id) {
        $('#' + id).html("");
    }

    function delete_indicators(id) {
        $('#' + id).html("");
        $('#indicators_delete').append('<input type="hidden" name="indicators_delete[]" value="' + id + '">');
    }
    ...
    $("#indicators_form").on('submit', (function(e) {
        ...
        $.ajax({
            url: base_url + "admin/grading_indicators/updateindicators",
            type: "POST",
