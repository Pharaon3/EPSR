...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="row">
            <?php if ($this->rbac->hasPrivilege('psychology_search', 'can_view')) { ?>
                <!-- start left column -->
                <div class="col-md-4">
                    <!-- Horizontal Form -->
                    <div class="box box-primary">
                        <div class="box-header with-border">
                            <h3 class="box-title" id="psychology_editbox_title"><?php echo $this->lang->line('add') . " " . $this->lang->line('Psychology'); ?></h3>
                        </div><!-- /.box-header -->
                        <form id="form1" action="<?php echo site_url('admin/psychology') ?>" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                            <div class="box-body">
                                <?php echo $this->session->flashdata('psychology_msg') ?>
                                <input type="hidden" id="psychology_id" name="psychology_id" value="0">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="box-body table-responsive">
                                    ...
                                    <div>
                                        <table class="table table-hover table-striped table-bordered psychology-list">
                                            <thead>
                                                <tr>
...
<script>
    ...
    $(document).ready(function() {
        ...
        emptyDatatable('psychology-list', 'data');
                ...
                success: function(response) { // your success handler
                    if (!response.status) {
                        ...
                        table = $('.student-list').DataTable({
                            ...
                            "serverSide": false,
                            "ajax": {
                                "url": baseurl + "admin/psychology/dtstudentlist",
                                "dataSrc": 'data',
                                "type": "POST",
    ...
    function selectePsychology(student_id, student_name, psychology_id, admission_no, guardian_name, guardian_phone1, guardian_phone2, guardian_email) {
        var base_url = '<?php echo base_url() ?>';
        ...
        $("#psychology_id").val(psychology_id);
        ...
        if (psychology_id == '0') { // add
            $('#psychology_editbox_title').html("<?php echo $this->lang->line('add') .' ' . $this->lang->line('psychology'); ?>");
            $("#report_list").css('display', 'block');
            $("#description").val("");
            if ($.fn.DataTable.isDataTable('.psychology-list')) { // if exist datatable it will destrory first
                $('.psychology-list').DataTable().destroy();
            }
            ...
            table = $('.psychology-list').DataTable({
                dom: 'Bfrtip',
                buttons: btn_format,
                "language": {
                    processing: '<i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">Loading...</span> '
                ...
                "serverSide": false,
                "ajax": {
                    "url": baseurl + "admin/psychology/getPsychologyListByStudentId",
                    "dataSrc": 'data',
                    "type": "POST",
                    'data': params,
                },
            ...
            $('#psychology_editbox_title').html("<?php echo $this->lang->line('edit') .' ' . $this->lang->line('psychology'); ?>" + ": #" + psychology_id);
            var base_url = '<?php echo base_url() ?>';
            $.ajax({
                type: "POST",
                url: base_url + "admin/psychology/getPsychology",
                data: {
    ...
    function showPsychologyList(student_id, student_name, guardian_name, guardian_phone1, guardian_phone2, guardian_email) {
        var base_url = '<?php echo base_url() ?>';
        $("#report_list").css('display', 'block');
        if ($.fn.DataTable.isDataTable('.psychology-list')) { // if exist datatable it will destrory first
            $('.psychology-list').DataTable().destroy();
    ...
    function deletePsychology(student_id, student_name, psychology_id, admission_no, guardian_name, guardian_phone1, guardian_phone2, guardian_email) {
        var base_url = '<?php echo base_url() ?>';
        var delete_str = "<?php echo $this->lang->line('delete_confirm') ?>";
        if (confirm(delete_str)) {
            $.ajax({
                type: "POST",
                url: base_url + "admin/psychology/deletePsychology",
                data: {
                    'psychology_id': psychology_id
                },
                dataType: "json",
                success: function(data) {
                    if (data.status == "success") {
                        showPsychologyList(student_id, student_name, admission_no, guardian_name, guardian_phone1, guardian_phone2, guardian_email);
                    }
                }
            });
        }
